...
class StudentDAO:
    def __init__(self):
        ...
        with open("students.csv", mode = 'r+') as StudentFile:
            for line in StudentFile.read()[:-1].split("\n"):
    ...
    def validate_user(self, email, pw):
        for i in self.__student_list:  
            if i.email == email and i.s_password == pw: 
                return True
        return False
...
class AttendingDAO:
    def __init__(self):
        ...
        with open("attending.csv", mode = 'r+') as attendingFile:
            for line in attendingFile.read()[:-1].split("\n"):
                course_id, student_email =line.split(',')
                
                a = Attending(course_id, student_email)
